...
def XYZ2Lab(X, Y, Z):  # XYZ空间转Lab空间，返回一个Lab的list
...
while True:
    ...
    Xce, Yce, Zce = map(float, input('输入XYZ并以空格隔开:').split())  # 获取需要转换的XYZ
    print("计算中……")
    ...
    for c in range(0, 101, 1):
        for m in range(0, 101, 1):
            for y in range(0, 101, 1):
                ...
                if deltaE_temp < deltaE:  # 找到最小的色差所对应的cmy
                    deltaE = deltaE_temp
                    c_result = c
                    m_result = m
                    y_result = y
